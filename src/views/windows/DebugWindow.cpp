...
DebugWindow::DebugWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::DebugWindow) {
	...
	connect(nodeView, &QTreeWidget::itemSelectionChanged, this, &DebugWindow::onItemClicked);

	dataView = this->findChild<QTabWidget *>("dataView");
	dataView->setTabsClosable(true);
	dataView->setMovable(true);
	connect(dataView, &QTabWidget::tabCloseRequested, this, &DebugWindow::onTabCloseRequest);
...
void DebugWindow::addTab(QWidget *widget, std::shared_ptr<SceneObject> object){
	for(auto& [o, w] : tabs){
		if(o == object){
...
void DebugWindow::onItemClicked() {
...
void DebugWindow::addNode(std::shared_ptr<IRenderNode> node, QTreeWidgetItem *parent) {
...
void DebugWindow::setScene(std::shared_ptr<Galax::Renderer::Scene> scene,
...
void DebugWindow::addLighting(std::shared_ptr<Galax::Renderer::LightingModel> model) {
...
void DebugWindow::addEffects(const std::vector<std::shared_ptr<Galax::Renderer::IPostProcessEffect>> &effects) {
	auto root = new QTreeWidgetItem(nodeView);
...
std::string DebugWindow::demangle(const char *name) {
...
void DebugWindow::onTabCloseRequest(int index) {
